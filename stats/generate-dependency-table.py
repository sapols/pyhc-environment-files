...
def get_other_pyhc_packages():
    ...
    ACEmag, AFINO, Auroral Electrojet (AEindex), geodata, GIMAmag (gima-magnetometer), POLAN (archived 2022), PyGemini,
    pyglow, python-magnetosphere, sami2py, scanning-doppler-interferometer (archived 2022).
    :return: A list of the other non-core PyHC package names.
    """
    return ["aacgmv2", "aiapy", "aidapy", "apexpy", "astrometry-azel", "cdflib", "dascutils", "dbprocessing", "dmsp", "enlilviz", "fiasco", "fisspy", "geopack", "georinex", "geospacelab", "ncarglow", "goesutils", "hissw", "hwm93", "igrf", "iri2016", "iri90", "irispy-lmsal", "lowtran", "madrigalWeb", "maidenhead", "mcalf", "mgsutils", "msise00", "ndcube", "nexradutils", "ocbpy", "OMMBV", "pydarn", "pyflct", "pymap3d", "pysatCDF", "pytplot", "pytplot-mpl-temp", "pyzenodo3", "reesaurora", "sciencedates", "SkyWinder", "SkyWinder-Analysis", "solarmach", "solo-epd-loader", "speasy", "spiceypy", "sunkit-image", "sunkit-instruments", "sunraster", "themisasi", "TomograPy", "viresclient", "wmm2015", "wmm2020"]
...
def get_dependency_ranges_by_package(packages, use_installed=False):
    ...
    for package in packages:
        ...
        all_dependencies[package] = sorted_dependencies
...
if __name__ == '__main__':
    ...
    # table_data = generate_dependency_table_data(other_packages, core_packages)
    all_packages = core_packages + other_packages
    table_data = generate_dependency_table_data(all_packages)

    # Write table_data
    # import pickle
    # with open('pyhc-dependency-table-may-11-2023.pkl', 'wb') as f:
    #     pickle.dump(table_data, f)

    # Read table_data
    # import pickle
    # with open('table_data_9_all_other_packages.pkl', 'rb') as f:
    ...
    table.save('pyhc-dependency-table-may-11-2023.xlsx')
    print("done")
